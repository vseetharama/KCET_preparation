...
// Initialize backend endpoint if not already set
if (!Store.get('ragConfig')) {
  Store.set('ragConfig', { endpoint: 'http://127.0.0.1:8000', apiKey: '' });
}
...
if (document.getElementById('dropZone')) {
  ...
  window.uploadToRAG = async () => {
    ...
    try {
      ...
      const uploadRes = await Promise.race([
        ...
        new Promise((_, reject) => setTimeout(() => reject(new Error('Upload timeout after 5 minutes')), 300000))
      ]);
      ...
      const debugRes = await fetch(`${endpoint}/health`);
      ...
      showToast(`✅ Ready to generate! (${debugData.chunks_indexed} content chunks loaded)`, 'success');
      ...
      if (e.message.includes('Failed to fetch')) {
        friendlyMsg = '❌ Cannot reach backend. Is the FastAPI server running? http://localhost:8000 should be accessible.';
  ...
  window.generatePapers = async () => {
    ...
    try {
      ...
      // Log questions per set
      data.sets.forEach((set, i) => {
        console.log(`   Set ${String.fromCharCode(65+i)}: ${set ? set.length : 0} questions`);
      });
      ...
      showToast(`❌ ${e.message}. Check console (F12) for details.`, 'error');
      console.log('💡 Possible fixes:\n1. Make sure papers were uploaded successfully\n2. Check backend logs in notebook\n3. Restart the notebook if timeout occurred');
      generatedSets = [];
    }
    ...
    if (totalQuestions === 0) {
      showToast('❌ No questions generated. Check backend logs in notebook.', 'error');
      console.error('❌ All sets are empty:', generatedSets);
      console.log('💡 Debug: Generated sets:', JSON.stringify(generatedSets, null, 2));
      console.log('💡 Troubleshooting:\n  • Look at notebook cell 8 output for error messages\n  • Check if "Generating Set A/B/C/D" messages appear\n  • Verify papers were uploaded successfully\n  • The Groq API key may be expired or invalid');
      return;
    }
